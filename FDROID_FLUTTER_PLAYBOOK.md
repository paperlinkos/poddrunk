...
- [ ] **`android/build.gradle.kts`**: Older pub.dev packages (e.g. `on_audio_query_android`) miss the `namespace` field in their `build.gradle`, causing AGP 8+ builds on F-Droid to fail with `Namespace not specified`.
...
Upon official merge and publishing of Poddrunk on F-Droid, this playbook will be packaged into a custom reusable Skill under:
